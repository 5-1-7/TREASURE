 ...
 * 调用数据层方法操作数据……
 * 有可能是一个数据层方法，
 * 涉及到复杂业务时，一个业务层对象里面有可能调用多个数据层方法
 * 以后发短信，发邮件也是在本类中实现
 * 2)拓展业务：
 * 日志记录(比如日志谁看过)、权限控制(删除是有权限吗)、
 * 事务控制（insert update）、为了提高性能加缓存……
 *
 * 注解 @Service 描述的对象表示是spring容器中的一个bean对象
 *
 * 练习 on 2021/3/10 19:11
 */
@Slf4j
@Service
public class NoticeServiceImpl implements NoticeService {
    ...
    @Override
    public List<NoticePojo> select(NoticePojo notice) {
        ...
        List<NoticePojo> list = noticeDao.select(notice);
    ...
    @Override
    public NoticePojo selectById(Long id) {
